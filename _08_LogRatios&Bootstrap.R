...
interaction_Age_FuncRole_RSN <- function(cluster1, cluster2, max, min, max2, min2, alpha, RSN_modular, RSN_interareal) {
  ...
  delta_proportion_a <<- tmp_cluster_final %>%
    ...
    summarise_at(vars(freq), funs(geomMeanExtension(., epsilon = 1e-5))) %>% 
...
interaction_Age_FuncRole_RSN("Young", "Old", 1, -1, 1, -1, 0.1,
                             RSN_modular = "CON|DMN|FPN|Language|SMN",
...
bootstrap_ci <- function(n_boot, cluster1, cluster2, RSN_modular, RSN_interareal) {
  ...
  for (i in 1:length(data_boot)) {
    ...
    delta_boot_a <- rbindlist(list_boot_bis) %>% as.data.frame() %>%
  ...
  for (i in 1:length(data_boot_bis)) {
    ...
    delta_boot_b <- rbindlist(list_boot_bis) %>% as.data.frame() %>%
      pivot_longer(cols = !c("n_data_boot", "n_data_split", "1st_network", "Subj_ID", "Age_group"), names_to = "Bridgeness", values_to = "freq") %>% 
      group_by(`1st_network`, Age_group, Bridgeness) %>%
      summarise_at(vars(freq), funs(geomMeanExtension(., epsilon = 1e-5))) 
...
bootstrap_ci(n_boot = 1000, "Young", "Old",
