...
data_coda_modular <- TFP_General %>% filter(Gender != "NaN" & Age != "NaN") %>% 
  dplyr::select(Connector, Satellite, Provincial, Peripheral) %>% acomp(.)

data_coda_interareal <- TFP_General %>% filter(Gender != "NaN" & Age != "NaN") %>%
  dplyr::select(Global_Bridge, Local_Bridge, Super_Bridge, Not_a_Bridge) %>% acomp(.)
