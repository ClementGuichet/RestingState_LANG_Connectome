...
imputated_data <- cmultRepl(data_compositional)
imputated_data_bis <- cmultRepl(data_compositional_bis)



# data_ilr_bis <- acomp(data_compositional_bis) # Declare the dataset to be compositional
...
# Then , robust principal component analysis is performed with a robust covariance estimator MCD. 
# Resulting loadings and scores are back-transformed to the clr space where the compositional biplot can be shown.
...
# CLR removes the value-range restriction but not the unit-sum constraint which makes PCA sensitive to
# outliers 
# --------------------------------------------------------------------------
# Keeping 5 log contrasts which explain 90% of variance
...
cv <- robustbase::covMcd(data, cor = FALSE)
pcaIlr <- princomp(data, covmat = cv, cor = FALSE)
summary(pcaIlr)
# Ilr-robust biplot
biplot(pcaIlr, scale = 0)

data_cluster <- pcaIlr$scores %>% as.data.frame() %>% 
...
robCODA <- robCompositions::pcaCoDa(data_CODA, method = "robust", solve = "eigen",
                                    mult_comp = list(
